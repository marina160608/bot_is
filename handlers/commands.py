...
async def start_handler(message: Message):
    start_text = (
        "Привет! Я рад, что ты хочешь начать подготовку к итоговому сочинению, поэтому собираюсь помочь с выбором актуальных произведений для твоих аргументов 😉 \n"
        "Выбирай раздел и подраздел тем ИС, а я подберу для тебя произведения разных форматов 👌🏻"
        "Нажимай на нужную команду 👇🏻\n"
        '/about - мое описание\n'
        '/help - помощь\n'
        '/task - описание задания на экзамене\n'
        '/criteria - критерии сочинения\n'
        '/literature - список произведений\n'
        '/advice - советы, которые помогут на экзамене\n'
    )
...
async def about_IS_hendler(message: Message):
    await message.answer(text = 'Выберите только ОДНУ из предложенных тем итогового сочинения, в бланке записи итогового сочинения перепишите название выбранной темы сочинения.'
    'Напишите сочинение-рассуждение на эту тему. Рекомендуемый объём − от 350 слов. Если в сочинении менее 250 слов (в подсчёт включаются всеслова, в том числе и служебные),'
    'то за такую работу ставится «незачёт». В рамках заявленной темы сформулируйте свою позицию, докажитееё, подкрепляя аргументы примерами из опубликованных литературных'
    'произведений. Можно привлекать произведения устного народного творчества (за исключением малых жанров), а также художественную, документальную,'
    ...
    'Продумайте композицию сочинения. Соблюдайте речевые и орфографические нормы (разрешается пользоваться орфографическим словарём). '
    'Сочинение пишите чётко и разборчиво. При оценке сочинения в первую очередь учитывается соблюдение требований объема и самостоятельности'
    'написания сочинения, соответствие выбранной теме, умение аргументировать позицию и обоснованно привлекать литературный материал.')
...
async def criteria_hendler(message: Message):
    await message.answer(text='Самые важные критерии:\n'
                         ...
                         '- Композиция\n'
                         '- Качество речи\n '
                         '- Грамотность', reply_markup=criteria_doc)
...
async def advice_handler(message: Message):
    await message.reply(text='- Выберите тему, для которой у вас есть АРГУМЕНТ.\n'
                             '- Отталкивайтесь от аргумента, когда будете составлять тезис.\n'
                             '- Составьте план.\n'
                             '- Пишите проще.\n'
                             '- Тренируйтесь.\n'
...
async def about_message(message: Message):
    await message.reply(text="Я твой помощник в подготовке к итоговому сочинению по русскому языку! Помогу подобрать "
...
async def react_to_reply(message: Message):
    await message.answer(text='да не за что:)')
...
async def show_menu(message: Message):
    show_text = "Ты вызвал команду /menu \nТут можешь познакомиться с "
    await message.answer(text=show_text, reply_markup=menu_kb)
"""
